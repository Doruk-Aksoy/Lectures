...
node* largest_leaf(node* t) {
	node* temp = t;
	while(temp->right)
		temp = temp->right;
	if(is_leaf(temp))
		return temp;
	return largest_leaf(temp->left);
}
 
int isBST_Work(node* t, int min, int max) { 
  if (!t) 
     return 1;
  if (t->data < min || t->data > max)
     return 0; 
  return isBST_Work(t->left, min, t->data - 1) && isBST_Work(t->right, t->data + 1, max);
}

int isBST(node* t) { 
  return isBST_Work(t, INT_MIN, INT_MAX); 
} 
...
int main() {
	...
	insert(&tree, 1337);
	display_tree(tree);
	printf("This tree is a bst: %d", isBST(tree));
	node* test = create_bst(60);
	test->left = (node*) s_malloc(sizeof(node));
	test->left->data = 777;
	test->left->left = NULL;
	test->left->right = NULL;
	test->right = (node*) s_malloc(sizeof(node));
	test->right->data = 1;
	test->right->right = NULL;
	test->right->left = NULL;
	display_tree(test);
	printf("This tree is a bst: %d", isBST(test));
	return 0;
}
